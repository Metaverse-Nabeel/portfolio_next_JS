...
const MovingImage = ({title, img, link}) =>{
    ...
    return (
        <Link href={link} target="_blank" onMouseMove={handleMouse} onMouseLeave={handleMouseLeave}>
                ...
                <FramerImage ref={imgRef} style={{x:x, y:y}} intial={{opacity:0}} whileInView={{opacity:1, transition:{duration: 0.2}}} src={img} alt={title} className="z-10 w-96 h-auto hidden absolute rounded-lg"/>
        </Link>
    )
}
...
const Article = ({ img, title, date, link }) => {
    return (
        <motion.li intial={{y:200}} whileInView={{y:0, transition:{duration:0.5, ease:easeInOut}}} viewport={{once: true}} className="relateive w-full p-4 py-6 my-4 rounded-xl flex items-center justify-between bg-light first:mt-0 border border-solid border-dark border-r-4 border-b-4 dark:border-light dark:bg-dark">
